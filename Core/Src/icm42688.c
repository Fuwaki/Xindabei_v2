...
void my_spi_write(uint8_t reg, uint8_t value) {
...
uint8_t my_spi_read(uint8_t reg) {
    HAL_GPIO_WritePin(ICM_CS_GPIO_Port, ICM_CS_Pin, GPIO_PIN_RESET);
...
void my_spi_read_burst(uint8_t reg, uint8_t *data, uint8_t len) {
    HAL_GPIO_WritePin(ICM_CS_GPIO_Port, ICM_CS_Pin, GPIO_PIN_RESET);
...
icm42688_comm_t icm42688_comm = {.init = my_spi_init,
                                 .read_reg = my_spi_read,
                                 .write_reg = my_spi_write,
                                 .read_regs = my_spi_read_burst};

// 实现系统接口
icm42688_system_t icm42688_system = {.delay_ms = my_delay_ms};
...
void stm32_icm42688_hal_init(icm42688_config_t *config) {
  icm42688_hal_init(&icm42688_comm, &icm42688_system, config);
}
