...
- **EncPLL (编码器观测器)**: 改进的龙伯格观测器，高精度速度估算，有效滤除编码器原始数据的噪声。（PS：为什么命名是PLL但是是观测器，因为之前想用锁相环的思路发现不好用，最后内核换成了观测器，但是是命名懒得改了（雾））
- **多级 PID 控制**: 针对循迹偏差和姿态角进行精准控制。循迹环/角速环->角速度环->(电流环 未成功)->电机速度环

### 2. 智能场景识别 (C++ FSM)
- 基于 C++ 有限状态机，比c语言实现更能明确地管理状态：
  - 基础电磁循迹。
  - 环岛自动识别、预进入、环内行驶及退出逻辑。
  - 基于 TOF 传感器的障碍物检测与避障（实际上因为tof不好用用了定时停车）
  - 集成 IMU 翻车检测，异常情况自动紧急制动。
  - 停车状态，本来应该用干簧管检测磁铁停车，后来发现不好用换成了定时停车
